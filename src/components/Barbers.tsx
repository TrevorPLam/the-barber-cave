import { Star, Phone, Calendar, Award, Music, MapPin, Clock } from 'lucide-react';
import { barbers } from '@/data/barbers';
import { EXTERNAL_LINKS } from '@/data/constants';
import Image from 'next/image';
import SectionHeader from './SectionHeader';
import LinkWithIcon from './LinkWithIcon';

export default function Barbers() {
  // TODO: Replace with unique barber photos when available
  // Currently all barbers use the same placeholder image
  // Future: Add individual photos for each barber to showcase their work
  const barbersData = barbers;
  
  return (
    <section id="barbers" className="py-20 bg-gray-50">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <SectionHeader
          title="Master Barbers"
          description="Meet our team of expert barbers, each with their own unique style and expertise"
        />
        
        <div 
          style={{ containerType: 'inline-size' }}
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8"
        >
          {barbersData.map((barber) => (
            <div key={barber.id} className="barber-card text-center group container-card">
              <div className="relative mb-6 overflow-hidden rounded-2xl bg-gray-200 aspect-square">
                <Image 
                  src={barber.image}
                  alt={barber.name}
                  fill
                  quality={75}
                  sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw"
                  className="object-cover group-hover:scale-105 transition-transform duration-300"
                />
                <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-sm rounded-lg p-3">
                  <p className="text-xs text-green-600 font-semibold">Available {barber.available}</p>
                </div>
              </div>
              
              <h3 className="text-xl font-bold text-black mb-1">{barber.name}</h3>
              <p className="text-gray-600 mb-2">{barber.title}</p>
              
              {/* Rating and Reviews Display */}
              <div className="flex items-center justify-center space-x-2 mb-4">
                <div className="flex items-center space-x-1">
                  <Star className="h-4 w-4 text-amber-500 fill-current" />
                  <span className="text-sm font-semibold text-gray-900">
                    {barber.rating === null ? 'New' : barber.rating}
                  </span>
                </div>
                {barber.reviews !== '0' && (
                  <span className="text-sm text-gray-500 font-medium">
                    ({barber.reviews})
                  </span>
                )}
              </div>
              
              {/* Enhanced Details */}
              <div className="space-y-2 text-sm">
                {barber.specialties && (
                  <div className="flex flex-wrap gap-1 justify-center">
                    {barber.specialties.slice(0, 2).map((specialty, index) => (
                      <span key={index} className="bg-gray-100 text-gray-700 px-2 py-1 rounded-full text-xs">
                        {specialty}
                      </span>
                    ))}
                  </div>
                )}
                
                {barber.achievements && barber.achievements.length > 0 && (
                  <div className="flex items-center justify-center text-amber-600">
                    <Award className="h-3 w-3 mr-1" />
                    <span className="text-xs font-medium">{barber.achievements[0]}</span>
                  </div>
                )}
                
                {barber.upcomingEvent && (
                  <div className="bg-red-50 text-red-700 p-2 rounded-lg text-xs font-medium">
                    <Calendar className="h-3 w-3 inline mr-1" />
                    {barber.upcomingEvent}
                  </div>
                )}
                
                {barber.vipContact && (
                  <div className="bg-yellow-50 text-yellow-800 p-2 rounded-lg">
                    <div className="font-bold text-xs mb-1">VIP Monday Service</div>
                    <a href={`tel:${barber.vipContact}`} className="flex items-center justify-center hover:text-yellow-900">
                      <Phone className="h-3 w-3 mr-1" />
                      <span className="text-xs font-medium">{barber.vipContact}</span>
                    </a>
                  </div>
                )}
                
                {barber.spotify && (
                  <div className="flex items-center justify-center text-purple-600">
                    <Music className="h-3 w-3 mr-1" />
                    <span className="text-xs">Spotify Artist</span>
                  </div>
                )}
                
                {barber.personalBrand && (
                  <div className="text-gray-500 text-xs italic">
                    {barber.personalBrand}
                  </div>
                )}
                
                {barber.availability && barber.availability !== barber.available && (
                  <div className="flex items-center justify-center text-gray-600">
                    <Clock className="h-3 w-3 mr-1" />
                    <span className="text-xs">{barber.availability}</span>
                  </div>
                )}
              </div>
              
              {/* Social Links */}
              <div className="mt-4 flex justify-center space-x-2">
                {barber.instagram && (
                  <a
                    href={`https://instagram.com/${barber.instagram.replace('@', '')}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-gray-100 text-gray-600 p-2 rounded-full hover:bg-gray-200 transition-colors"
                    title={barber.instagram}
                  >
                    <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zM5.838 12a6.162 6.162 0 1112.324 0 6.162 6.162 0 01-12.324 0zM12 16a4 4 0 110-8 4 4 0 010 8zm4.965-10.405a1.44 1.44 0 112.881.001 1.44 1.44 0 01-2.881-.001z"/>
                    </svg>
                  </a>
                )}
                {barber.facebook && (
                  <a
                    href={`https://facebook.com/${barber.facebook}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-gray-100 text-gray-600 p-2 rounded-full hover:bg-gray-200 transition-colors"
                    title="Facebook"
                  >
                    <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                    </svg>
                  </a>
                )}
              </div>
            </div>
          ))}
        </div>
        
        <div className="text-center mt-12">
          <LinkWithIcon
            href={EXTERNAL_LINKS.booking}
            target="_blank"
            rel="noopener noreferrer"
          >
            Book With Our Barbers
          </LinkWithIcon>
        </div>
      </div>
    </section>
  );
}
